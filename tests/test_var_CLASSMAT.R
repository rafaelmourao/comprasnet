...
PROD_FREQ  <- sort(table(F_ITEM_COMPRA$ID_ITCP_CLASSE_MAT_SERV))
...
# Record frequency for residual variance for each category, and select the top 100
...
for (i in 1:100) {
    ...
    most_freq_table  <- F_ITEM_COMPRA[as.integer(ID_ITCP_CLASSE_MAT_SERV) == as.integer(most_frequent)]
    ...
    if (length(unique(most_freq_table$ID_ITCP_TP_COD_MAT_SERV)) > 1 &&
      ...
      NEW_REG  <- lm(log(VL_PRECO_TOTAL_HOMOLOG)~log(VL_ITCP_PRECO_GLOBAL_ESTIM)+PRESENCIAL+SRP+
                       ME_EX+QT_FORNEC_PARTICIP_ITEM+I(QT_FORNEC_PARTICIP_ITEM^2)+
                       log(QT_ITCP_SOLICITADA)+factor(DS_LCAL_UF_UNIDADE)+factor(ANO)+
                       factor(ID_ITCP_TP_COD_MAT_SERV),data=most_freq_table)
    } else {
      NEW_REG  <- lm(log(VL_PRECO_TOTAL_HOMOLOG)~log(VL_ITCP_PRECO_GLOBAL_ESTIM)+PRESENCIAL+SRP+
                       ME_EX+QT_FORNEC_PARTICIP_ITEM+I(QT_FORNEC_PARTICIP_ITEM^2)+
                       log(QT_ITCP_SOLICITADA)+factor(DS_LCAL_UF_UNIDADE)+factor(ANO),
  ...
  most_freq_table[,resid := resid(NEW_REG)]
  prod_var[i,"variance_2"]  <- var(resid(NEW_REG))
  ...
  # Plot new residual distributions
  setorder(table_uf_ano,variance)
  png(paste0("figures/var_classmat/test_var_CLASSMAT_ANO_",i,".png"))
  plot_title  <- paste(strwrap(F_ITEM_COMPRA[ID_ITCP_CLASSE_MAT_SERV == most_frequent,
                                             DS_ITCP_CLASSE_MAT_SERV][1],width=50),collapse="\n")
  plot(B$DS_LCAL_UF_UNIDADE,table_uf_ano$variance,main = plot_title,las=2,cex.axis=0.7)
  dev.off()

  png(paste0("figures/var_classmat/no_outline_test_var_CLASSMAT_ANO_",i,".png"))
  plot_title  <- paste(strwrap(F_ITEM_COMPRA[ID_ITCP_CLASSE_MAT_SERV == most_frequent,
                                             DS_ITCP_CLASSE_MAT_SERV][1],width=50),collapse="\n")
  plot(B$DS_LCAL_UF_UNIDADE,table_uf_ano$variance,main = plot_title,las=2,cex.axis=0.7,
       outline = FALSE)
  ...
  pregoes[[i]] <- most_freq_table[,.(ID_ITCP_ITEM_COMPRA,ID_ITCP_TP_COD_MAT_SERV,
                                     DS_ITCP_PADRAO_DESC_MAT,DS_ITCP_CLASSE_MAT_SERV,
                                     DS_LCAL_UF_UNIDADE,DS_LCAL_REGIAO_UNIDADE,
                                     DT_REFERENCIA_COMPRA,ANO,resid)]
}

setorder(prod_var,variance)
# save data for further analysis
save(list = c("summaries", ls(pattern="var*"), "pregoes", "prod_var"), 
     file = "test_var_classmat.RData")
# write table with variance results
write.table(prod_var, "prod_var_classmat.txt", sep="\t")
