...
ORG_REG  <- lm(log(VL_PRECO_UNIT_HOMOLOG)~log(VL_ITCP_PRECO_UNIT_ESTIM)+PRESENCIAL+SRP+
...
prod_var <- F_ITEM_COMPRA[,.(freq = .N, variance = var(lm_resid), variance_2=0),
                          by = .(ID_ITCP_TP_COD_MAT_SERV,DS_ITCP_MATERIAL_SERVICO)]
...
for (i in 1:100) {
    ...
    most_freq_table  <- F_ITEM_COMPRA[ID_ITCP_TP_COD_MAT_SERV == as.integer(most_frequent)]
    most_freq_table <- droplevels(most_freq_table)
    
    # Differently from the other cases, the CATMAT has not subcategories
    ...
    png(paste0("figures/var_catmat/test_var_CATMAT_UF_ANO_",i,".png"))
    plot_title  <- paste(strwrap(F_ITEM_COMPRA[ID_ITCP_TP_COD_MAT_SERV == most_frequent,
                                               DS_ITCP_MATERIAL_SERVICO][1],width=50),collapse="\n")
    plot(B$DS_LCAL_UF_UNIDADE,table_uf_ano$variance,main = plot_title,las=2,cex.axis=0.7)
    dev.off()

    png(paste0("figures/var_catmat/no_outlier_test_var_CATMAT_UF_ANO_",i,".png"))
    plot_title  <- paste(strwrap(F_ITEM_COMPRA[ID_ITCP_TP_COD_MAT_SERV == most_frequent,
                                               DS_ITCP_MATERIAL_SERVICO][1],width=50),collapse="\n")
    plot(B$DS_LCAL_UF_UNIDADE,table_uf_ano$variance,main = plot_title,las=2,cex.axis=0.7,
         outline=FALSE)
    ...
    png(paste0("figures/var_catmat/test_var_CATMAT_UF_",i,".png"))
    ...
    variances_ano[[i]]  <- table_uf_ano
    pregoes[[i]] <- most_freq_table[,.(ID_ITCP_ITEM_COMPRA,ID_ITCP_TP_COD_MAT_SERV,
                                       DS_ITCP_TP_MATERIAL_SERVICO,DS_LCAL_UF_UNIDADE,
...
write.table(prod_var, "prod_var_catmat.txt", sep="\t")
# write table with variance results
save(list = c("summaries", ls(pattern="var*"), "pregoes", "prod_var"), file = "test_var_catmat.RData")
